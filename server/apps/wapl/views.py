...
def create_private_plan(request, *args, **kwargs):
    if request.method == 'POST':
        ...
        result, err_msg = validate_plan(
            startTime=startTime, endTime=endTime, title=req['title'])

        if result:  # validation 통과한 경우
            new_plan = PrivatePlan.objects.create(
                owner=request.user, startTime=startTime, endTime=endTime, location=location, title=title, content=content)
            # 새로운 share 모델 생성
            for shareMeeting in shareMeetings:
                Share.objects.create(plan=new_plan, meeting=meetings.get(
                    meeting_name=shareMeeting), is_share=shareMeetings[shareMeeting])
...
def create_public_plan(request, *args, **kwargs):
    if request.method == 'POST':
        ...
        result, err_msg = validate_plan(
            startTime=startTime, endTime=endTime, title=req['title'])
        if result:
            new_plan = PublicPlan.objects.create(
                meetings=meeting, owner=request.user, startTime=startTime, endTime=endTime, location=location, title=title, content=content)

            users = meeting.users.all()

            for user in users:
                Attend.objects.create(
                    plan=new_plan, user=user, is_attend="standby")
    ...
    for meeting in meeting_list:
        ...
        if not Share.objects.filter(meeting=meeting, plan=plan).exists():
            new_share = Share.objects.create(
                plan=plan, meeting=meeting, is_share='close')
...
def view_plan(request):
    ...
    private_plans = PrivatePlan.objects.filter(
        owner=login_user, startTime__year__lte=year, endTime__year__gte=year)
    ...
    for meeting in meetings:
        public_plan = PublicPlan.objects.all().filter(meetings=meeting, startTime__year__lte=year,
                                                      endTime__year__gte=year).filter(plan_attend__is_attend="standby", plan_attend__user=login_user)
        public_plans += list(public_plan)

    for meeting in meetings:
        public_plan = PublicPlan.objects.all().filter(meetings=meeting, startTime__year__lte=year,
                                                      endTime__year__gte=year).filter(plan_attend__is_attend="attend", plan_attend__user=login_user)
        public_plans += list(public_plan)
...
def view_team_plan(request):
    ...
    share_list += list(Share.objects.filter(meeting=meeting,
                       is_share="untitled"))
    ...
    public_plans = list(PublicPlan.objects.all().filter(
        meetings=meeting, startTime__year__lte=year, endTime__year__gte=year))
...
def view_explan(request):
    ...
    private_plans = PrivatePlan.objects.filter(
        owner=login_user, startTime__lte=today + timedelta(days=1), endTime__gte=today)
    ...
    for meeting in meetings:
        public_plan = PublicPlan.objects.all().filter(meetings=meeting, startTime__lte=today + timedelta(days=1),
                                                      endTime__gte=today).filter(plan_attend__is_attend="standby", plan_attend__user=login_user)
        public_plans += list(public_plan)

    for meeting in meetings:
        public_plan = PublicPlan.objects.all().filter(meetings=meeting, startTime__lte=today + timedelta(days=1),
                                                      endTime__gte=today).filter(plan_attend__is_attend="attend", plan_attend__user=login_user)
        public_plans += list(public_plan)
...
def view_team_explan(request):
    ...
    public_plans = PublicPlan.objects.filter(
        meetings=meeting, startTime__lte=today + timedelta(days=1), endTime__gte=today)
    ...
    share_plans = list(share_plans.filter(
        startTime__lte=today + timedelta(days=1), endTime__gte=today))
